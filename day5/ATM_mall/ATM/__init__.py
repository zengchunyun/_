...
from publicAPI import auth_account, register_account
...
def management_admin_account(admin_name, quit_managemnet_account):
    while not quit_managemnet_account:
        admin_database = user_info['admin_bank']
        print("""中国建都银行    管理中心
        ...
        if wait_choose == "1":
            ...
            quit_managemnet_account = True
            break
        elif str(wait_choose).lower() in ['b', 'back', ]:
            break
        else:
            print("操作有误 !!!")
    return quit_managemnet_account
